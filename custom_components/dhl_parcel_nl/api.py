...
class DHLParcelNLAPI:
    ...
    def __init__(
        ...
        self.consumer_auth_fail_count: int = 0
        self.consumer_auth_block_until: datetime | None = None
    ...
    async def authenticate(self) -> None:
        ...
        if self.account_type == ACCOUNT_TYPE_CONSUMER and self.token_expires_at:
            if datetime.now() < self.token_expires_at - timedelta(minutes=5):
                return

        if (
            self.account_type == ACCOUNT_TYPE_BUSINESS
    ...
    async def _authenticate_consumer(self) -> None:
        ...
        if (
            self.consumer_auth_block_until
            and datetime.now() < self.consumer_auth_block_until
        ):
            raise RuntimeError(
                "Consumer authentication temporarily blocked after repeated failures"
            )

        if not self.email or "@" not in self.email:
            raise ValueError("Consumer email format is invalid")
        ...
        try:
            async with asyncio.timeout(10):
                ...
                self.consumer_auth_fail_count = 0
                self.consumer_auth_block_until = None
                _LOGGER.debug("Successfully authenticated with consumer DHL API")

        except ClientResponseError as err:
            self.consumer_auth_fail_count += 1
            if self.consumer_auth_fail_count >= 3:
                self.consumer_auth_block_until = datetime.now() + timedelta(minutes=30)
                _LOGGER.warning(
                    "Consumer login blocked for 30 minutes after repeated failures"
                )
    ...
    async def _get_consumer_shipments(self) -> list[str]:
        ...
        try:
            data = await self._fetch_consumer_parcels()
        except ClientResponseError as err:
            if err.status != 401:
                raise

            _LOGGER.warning(
                "Consumer parcels request returned 401; re-authenticating and retrying once"
            )
    ...
    async def _fetch_consumer_parcels(self) -> dict[str, Any] | list[dict[str, Any]]:
        """Fetch consumer parcels payload from DHL portal API."""
        headers = self._build_consumer_headers()
        _LOGGER.debug(
            "Consumer parcels request headers prepared (bearer=%s, xsrf=%s)",
            bool(self.access_token),
            "x-xsrf-token" in headers,
        )
    ...
    def _build_consumer_headers(self) -> dict[str, str]:
        """Build headers for consumer portal API calls."""
        headers = {"Accept": "application/json"}

        xsrf_token = self._get_consumer_xsrf_token()
        if xsrf_token:
    ...
    def _get_consumer_xsrf_token(self) -> str | None:
        """Get XSRF token from cookie jar when present."""
        cookies = self.session.cookie_jar.filter_cookies(CONSUMER_BASE_URL)
        for cookie_name in ("XSRF-TOKEN", "XSRF_TOKEN", "x-xsrf-token", "xsrf-token"):
